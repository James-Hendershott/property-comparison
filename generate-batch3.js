...
/**
 * generate-batch3.js
 * Adds 17 new Virginia property cards (p55–p71) to public/index.html.
...
const properties = require('./batch3-va');
...
function main() {
  ...
  const oldSub = '50 Properties \u00b7 WA + ID + MT + WY + CO + OR + NC';
  const newSub = '67 Properties \u00b7 WA + ID + MT + WY + CO + OR + NC + VA';
  ...
  // 4. Update footer tax note to include VA
  const oldTaxNote = 'NC 0.38\u20130.65%)';
  const newTaxNote = 'NC 0.38\u20130.65%, VA 0.27\u20130.72%)';
  if (html.includes(oldTaxNote)) {
    html = html.replace(oldTaxNote, newTaxNote);
    console.log('\u2713 Updated footer tax note with VA rates');
    changes++;
  } else {
  ...
  console.log(`  - ${properties.length} Virginia properties injected (p55\u2013p71)`);
}
